...
# **********************************************************
# * POST-EXECUTION EVENT TRIGGER                           *
# **********************************************************
resource "aws_cloudwatch_event_rule" "s3_object_created" {
  name        = "${local.environment}-s3-object-created"
  description = "Triggers when new data is saved to S3 by the fetch_daily_summaries Lambda."
  ...
  event_pattern = jsonencode({
    ...
    "detail": {
      "eventName": ["PutObject"],
      "requestParameters": {
        "bucketName": [aws_s3_bucket.atomiklabs_data_bucket.bucket],
        "key": [{
          "prefix": "${local.data_ingestion_key_prefix}/"
        }]
      }
    }
  })
...
resource "aws_cloudwatch_event_target" "trigger_glue_job" {
  ...
  target_id = "TriggerGlueJobForNewData"
  arn       = aws_glue_job.arxiv-to-parquet.arn

  role_arn = aws_iam_role.eventbridge_glue_invocation_role.arn
}

resource "aws_iam_role" "eventbridge_glue_invocation_role" {
  name = "${local.environment}-eventbridge-to-glue-role"
...
resource "aws_iam_policy" "eventbridge_glue_invocation_policy" {
  name   = "${local.environment}-eventbridge-glue-invocation-policy"
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [{
      Action = "glue:StartJobRun",
      Resource = aws_glue_job.arxiv-to-parquet.arn,
      Effect = "Allow",
    }],
  })
}

resource "aws_iam_role_policy_attachment" "eventbridge_glue_policy_attach" {
  role       = aws_iam_role.eventbridge_glue_invocation_role.name
  policy_arn = aws_iam_policy.eventbridge_glue_invocation_policy.arn
}
