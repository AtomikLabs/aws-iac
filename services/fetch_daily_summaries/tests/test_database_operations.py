...
class TestGetEarliestUnfetchedDate(unittest.TestCase):
    ...
    def test_successful_retrieval(self, mock_boto3_client):
        ...
        mock_client.execute_statement.return_value = {
            'records': [{'fetch_date': past_dates[1]},
                        {'fetch_date': past_dates[2]}]
        }

        expected_earliest_date = min(set(past_dates) -
                                     {past_dates[1], past_dates[2]})
        result = get_earliest_unfetched_date('mock_aurora_arn',
                                             'mock_secret_arn',
                                             'mock_database')
...
class TestSetFetchStatus(unittest.TestCase):
    ...
    def test_set_fetch_status_success(self, mock_boto3_client):
        ...
        result = set_fetch_status(date, status, aurora_cluster_arn,
                                  db_credentials_secret_arn, database)
