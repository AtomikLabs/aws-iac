...
from services.fetch_daily_summaries.src.fetch_daily_summaries \
    import attempt_fetch_for_dates, handle_http_error, fetch_http_response, \
    extract_resumption_token, fetch_data
...
class TestAttemptFetchForDates(unittest.TestCase):
    ...
    def test_successful_fetch(self, mock_process_fetch,
                              mock_insert_fetch_status,
                              mock_generate_date_list, mock_fetch_data):
        ...
        mock_fetch_data.return_value = ['mock_xml_response_1',
                                        'mock_xml_response_2']
        ...
        mock_generate_date_list.assert_called_once_with('2023-01-01',
                                                        '2023-01-02')
        mock_insert_fetch_status.assert_any_call(
            '2023-01-01',
            aurora_cluster_arn,
            db_credentials_secret_arn,
            database)
        mock_insert_fetch_status.assert_any_call('2023-01-02',
                                                 aurora_cluster_arn,
                                                 db_credentials_secret_arn,
                                                 database)
...
class TestFetchData(unittest.TestCase):
    ...
    def test_successful_fetch_with_no_resumption_token(
            self, mock_extract_resumption_token, mock_fetch_http_response):
    ...
    def test_http_error_with_backoff(self, mock_handle_http_error,
                                     mock_fetch_http_response):
        mock_fetch_http_response.side_effect = [(500, 'Server Error'),
                                                (200, self.XML_CONTENT)]
    ...
    @patch(TIME_SLEEP_PATH, Mock())
    def test_data_fetch_with_resumption_token(self,
                                              mock_extract_resumption_token,
                                              mock_fetch_http_response):
        mock_fetch_http_response.side_effect = [
            (200, '<xml>mock_content_1</xml>'),
            (200, '<xml>mock_content_2</xml>')]
        mock_extract_resumption_token.side_effect = ['token', None]

        result = fetch_data('mock_base_url', '2023-01-01', 'mock_summary_set')

        self.assertEqual(result, ['<xml>mock_content_1</xml>',
                                  '<xml>mock_content_2</xml>'])
...
class TestHandleHttpError(unittest.TestCase):
    ...
    def test_retry_logic_503_first_try(self, mock_logging):
        ...
        backoff_time = handle_http_error(status_code, 'Service Unavailable',
                                         retry_count)
    ...
    def test_no_retry_logic_other_status_codes(self, mock_logging):
        ...
        backoff_time = handle_http_error(status_code, 'Server Error',
                                         retry_count)
    ...
    def test_maintenance_response(self, mock_schedule_for_later, mock_logging):
        ...
        backoff_time = handle_http_error(status_code, response_text,
                                         retry_count)
...
class TestFetchHttpResponse(unittest.TestCase):
    ...
    def test_successful_response(self, mock_requests_get):
        ...
        status_code, response_text = fetch_http_response('mock_base_url', 
                                                         {'param': 'value'})
...
class TestExtractResumptionToken(unittest.TestCase):
    ...
    def test_successful_token_extraction(self, mock_fromstring):
        ...
        token = extract_resumption_token('<xml><resumptionToken>\
