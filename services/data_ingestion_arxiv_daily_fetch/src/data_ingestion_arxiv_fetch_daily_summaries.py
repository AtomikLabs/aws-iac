...
def initialize_db() -> (psycopg2.extensions.connection,
    ...
    try:
        cursor.execute(
            "SELECT fetch_date FROM research_fetch_status \
                WHERE fetch_date = ANY(%s::DATE[]) AND \
                    status = 'success'",
            (past_dates,),
        )
        fetched_dates = [
...
def insert_fetch_status(date, cursor):
    ...
    cursor.execute(
        "INSERT INTO research_fetch_status (fetch_date, status) \
            VALUES (%s, 'pending') ON CONFLICT (fetch_date) DO NOTHING",
        (date,),
    )
...
def handle_http_error(status_code: int, response_text: str,
    ...
    if status_code == 503 and retry_count < len(backoff_times):
        logging.info(
            f"Received 503, retrying after {backoff_times[retry_count]} seconds"
        )
...
def set_fetch_success(date, cursor):
    ...
    """
    cursor.execute(
        "UPDATE research_fetch_status SET status = 'success' WHERE \
            fetch_date = %s",
        (date,),
    )
...
def set_fetch_failure(date, cursor):
    ...
    """
    cursor.execute(
        "UPDATE research_fetch_status SET status = 'failure', \
            retry_count = retry_count + 1 WHERE fetch_date = %s",
        (date,),
    )
